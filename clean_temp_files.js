...
var currentFiles = [localK3FilesPath];
...
var currentFolders = currentFiles.filter(function(fileName) {
	...
	var isDir;
	try {
		isDir = fs.statSync(path.join(fileName)).isDirectory();
	} catch(err) {
		return false;
	}
...
function printStatus() {
	...
	installationStates.forEach(function(install) {
		...
		message += '\t| ' + install.isCleaned.toString();
...
function updateStatus(name, options) {
	installationStates.forEach(function(install) {
		if(install.folder === name) {
			...
			if(typeof(options.isCleaned) !== 'undefined') {
				install.isCleaned = options.isCleaned;
			}
...
currentFolders.forEach(function(folder) {
	...
	try {
		...
		// console.log('Folder status', fs.existsSync(folder));
		console.log('Operating on folder', folder);
		// if(fs.existsSync(folder)) {
		// 	var rmOutput = fse.removeSync(folder);
		// } else {
		// 	console.log('folder does not exist', folder);
		// }
		var installOutput = child_process.execSync('rm -r K3');
		updateStatus(folder, {isCleaned: true, isSuccessful: true, isFinished: true});
	} catch(err) {
		if(err.code === 'ENOENT') {
			updateStatus(folder, {isCleaned: true, isSuccessful: true, isFinished: true});
		} else {
