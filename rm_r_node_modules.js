...
/*
 * rm_r_node_modules.js is a file that helps initialize the kipling development environment
 * for developers wanting to contribute to the project.  This specific script
 * goes into each folder and performs the commands "rm -r node_modules" 
 * to automate the deletion of each projects dependencies.
 *
 * Dependencies:
...
function printStatus() {
	...
	installationStates.forEach(function(install) {
		...
		message += '\t| ' + install.isDeleted.toString();
	...
	installationStates.forEach(function(install) {
		if(install.folder === name) {
			...
			if(typeof(options.isInstalled) !== 'undefined') {
				install.isInstalled = options.isDeleted;
			}
...
currentFolders.forEach(function(folder) {
	...
	try {
		...
		updateStatus(folder, {isDeleted: true, isSuccessful: true, isFinished: true});
	} catch(err) {
		console.log('Error!!!');
